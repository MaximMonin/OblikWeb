package com.as3xls.xls {
	...
	public class ExcelFile {
		...
		private function label(r:Record, s:Sheet):void {
			...
			var value:String = r.data.readMultiByte(len, "windows-1251");
			var fmt:String = s.formats[s.xformats[indexToXF].format];
			if(fmt == null || fmt.length == 0) {
				fmt = Formatter.builtInFormats[s.xformats[indexToXF].format];
			}
		...
		private function note(r:Record, s:Sheet):void {
			...
			if(version <= BIFFVersion.BIFF5) {
				...
				note = r.data.readMultiByte(r.data.bytesAvailable, "windows-1251");
		...
		private function boundsheet(r:Record, s:Sheet):void {
			...
			var name:String = r.data.readMultiByte(len, "windows-1251");
		...
		private function format(r:Record, s:Sheet):void {
			...
			var string:String = r.data.readMultiByte(len, "windows-1251");
			if(s is Sheet) {
		...
		private function header(r:Record, s:Sheet):void {
			...
			if(r.data.length == 0) {
				...
				var len:uint = r.data.readUnsignedByte();
				string = r.data.readMultiByte(len, "windows-1251");
			}
			s.header = string;
		}
		...
		private function footer(r:Record, s:Sheet):void {
			...
			var len:uint = r.data.readUnsignedByte();
			var string:String = r.data.readMultiByte(len, "windows-1251").substr(2); // Skip two bytes b/c of commands (left, center, etc)
			s.footer = string;
		}
		...
		private function writeaccess(r:Record, s:Sheet):void {
			var len:uint = r.data.readUnsignedByte();
			var username:String = r.data.readMultiByte(len, "windows-1251");
		}
		...
		private function cont(r:Record, s:Sheet):void {
			switch(lastRecordType) {
				case 0xEC:
					...
					var note:String = r.data.readMultiByte(r.data.bytesAvailable, "windows-1251");
					notes.push(note);
					break;
				default:
					break;
